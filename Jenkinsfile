script {
  ...
  // ----- KPIs bez regex Matcher w Groovy -----
  int passed = 0, failed = 0

  // 1) Spróbuj z output.xml -> Python zwraca "passed:failed"
  def pf = sh(
    script: '''python3 - <<'PY'
...
try:
    if os.path.isfile(p):
        ...
        if s is not None:
            pa=int(s.get('pass') or 0)
            fa=int(s.get('fail') or 0)
except Exception:
    pass
print(f"{pa}:{fa}")
PY
''',
  ...
  def parts = pf.tokenize(':')
  if (parts.size() == 2) {
    try { passed = parts[0] as int; failed = parts[1] as int } catch (ignored) {}
  }

  // 2) Fallback: odczytaj z konsoli Robota (awk robi regex POZA Groovy)
  if ((passed + failed) == 0 && fileExists("${env.ROBOT_DIR}/robot_console.txt")) {
    def pf2 = sh(
      script: "tac '${env.ROBOT_DIR}/robot_console.txt' | grep -m1 -E '[0-9]+ tests, [0-9]+ passed, [0-9]+ failed' | awk 'match(\$0,/([0-9]+) tests, ([0-9]+) passed, ([0-9]+) failed/,a){print a[2]\":\"a[3]; exit}' || true",
      returnStdout: true
    ).trim()
    def p2 = pf2.tokenize(':')
    if (p2.size() == 2) {
      try { passed = p2[0] as int; failed = p2[1] as int } catch (ignored) {}
    }
  }
